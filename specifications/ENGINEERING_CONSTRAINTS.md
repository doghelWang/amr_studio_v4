...
- **约束**：
  ...
  - **禁止猜测与创造 (No Guess / No Fabrication)**：
    - 严禁通过猜测、臆断、创造、假想的方式补齐参数值、描述信息、模块内容、能力内容或任何协议字段。
    - 当某个值、描述、类型、默认值、映射关系在事实源中不存在时，只允许执行以下动作之一：保持缺失、显式报错、标记为待确认、或使用已在规范中登记的兜底值。
    - 任何新增字段内容、中文描述、模块描述、子系统描述、接口说明，必须来自 Proto、Schema、模板库、明确的外部配置或用户输入，禁止“为了让流程跑通”临时编造。
    - 若实现层需要兜底，必须能追溯到正式事实源或正式规范条款，且应在审计或日志中可见。
