...
from app.application import project_management
        ...
        with tempfile.TemporaryDirectory() as temp_root:
            ...
            config = {
                ...
                "rawAbiSet": {
                    "version": "V9.9",
                    "functionAbility": [
                        {"type": "fixture", "desc": "source", "childFunction": []}
                    ],
                },
                "components": [
                    {
            ...
            try:
                with patch("app.application.project_management.split_comp_desc") as split_mock:
                    ...
                    result = project_management.initialize_project_sandbox("proj_diag", config)
                ...
                self.assertEqual(
                    json.loads((project_dir / "AbiSet.json").read_text(encoding="utf-8")),
                    config["rawAbiSet"],
                )
                ...
                self.assertTrue((debug_dir / "06_preserved_raw_AbiSet.json").exists())
            finally:
