...
from app.services.module_list_builder import MODULE_LIST_HEADERS, collect_module_rows, normalize_io_category
from core.fallback_diagnostics import collect_comp_desc_diagnostics
from skills_v2.cmodel_encoder.encoder import encode_cmodel, resolve_with_fidelity
...
def _load_blueprint(project_dir: Path) -> dict:
    ...
    if not blueprint_path.exists():
        raise HTTPException(status_code=400, detail=f"Project sandbox is missing blueprint_CompDesc.json: {project_dir.name}")
