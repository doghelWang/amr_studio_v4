...
from app.application.exceptions import InvalidRequestError, ResourceNotFoundError
...
from app.application.cmodel_export import collect_export_diagnostics, export_abilities, frontend_to_comp_desc
from app.infrastructure.protobuf import split_comp_desc
...
def initialize_project_sandbox(project_id: str, config: dict) -> dict:
    if not project_id or not config:
        raise InvalidRequestError("Missing projectId or config")
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        raw_abi_set = config.get("rawAbiSet")
        if isinstance(raw_abi_set, dict):
            abi_path = data_manager.get_project_dir(project_id) / "AbiSet.json"
            with open(abi_path, "w", encoding="utf-8") as file:
                json.dump(raw_abi_set, file, ensure_ascii=False, indent=2)
            write_debug_json(debug_artifact_dir, "06_preserved_raw_AbiSet.json", raw_abi_set)
        elif config.get("abilities"):
            abi_path = data_manager.get_project_dir(project_id) / "AbiSet.json"
...
def save_user_project_config(user_saves_dir: Path, name: str, config: dict) -> dict:
    if not name or not config:
        raise InvalidRequestError("Missing project name or config")
...
def load_user_project_config(user_saves_dir: Path, name: str) -> dict:
    ...
    if not file_path.exists():
        raise ResourceNotFoundError(f"Saved project not found: {name}")
