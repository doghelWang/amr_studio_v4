      ...
      <Row gutter={[16, 16]} className="workshop-grid">
        <Col xs={24} lg={6}>
          ...
          <Card title="装配清单" className="workshop-card installed-card">
            {installed.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="还没有装备，先从左侧选择模块" /> : <List size="small" dataSource={installed} renderItem={component => <List.Item title={`${componentOptionLabel(component)} · ${getCategoryLabel(component.category)} · ${positionState(component)}`} className={selectedInstalledId === component.id ? 'list-selected' : ''} onClick={() => selectInstalled(component)}>
        ...
        <Col xs={24} lg={6}>
          <Card title={<Space><ExperimentOutlined />模块库 · {FUNCTION_GROUPS.find(group => group.key === groupKey)?.label}</Space>} className="workshop-card catalog-card">
            ...
            {visibleCatalog.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={catalog.length === 0 ? <Space direction="vertical"><span>schemaRegistry 尚未加载，无法安全展示模块</span><Button size="small" icon={<ApiOutlined />} onClick={() => fetchSchemas()}>重新加载模块库</Button></Space> : '当前功能组没有可用模块'} /> : <List dataSource={visibleCatalog.slice(0, 30)} renderItem={item => <List.Item title={`${item.title || 'unknown'} · ${getCategoryLabel(item.category)}`} className={selectedCatalogId === item.id ? 'catalog-selected' : ''} onClick={() => setSelectedCatalogId(item.id)} actions={[<Button key="install" type={selectedCatalogId === item.id ? 'primary' : 'default'} size="small" icon={<PlusOutlined />} onClick={() => setSelectedCatalogId(item.id)}>选择</Button>]}> 
