...
# Registered LAST to avoid shadowing /api and /downloads
FRONTEND_DIST = BASE_DIR.parent / "frontend" / "dist"
if FRONTEND_DIST.exists():
    # Assets are specific enough to stay
    app.mount("/assets", StaticFiles(directory=str(FRONTEND_DIST / "assets")), name="assets")

    @app.get("/{full_path:path}")
    async def serve_frontend(full_path: str):
        # Explicit check to avoid accidental API shadowing
        if full_path.startswith("api") or full_path.startswith("downloads"):
            return JSONResponse(status_code=404, content={"error": "NotFound", "path": full_path})
    ...
    print(f"WARNING: Frontend dist not found at {FRONTEND_DIST}. Unified deployment disabled.", flush=True)

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="AMR Studio V4 Unified Server")
    ...
    print(f"🚀 Starting Unified Server on http://{args.host}:{args.port}", flush=True)
